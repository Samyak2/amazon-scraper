...
class AmazonSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if product["name"] != "N/A":
            ...
            deal_price = price_extract(response.xpath('//*[@id="priceblock_dealprice"]/text()').extract())
            ASIN = if_available_strip(response.xpath('//*[@id="prodDetails"]//td[text()="ASIN"]/following::td[@class="value"][1]/text()').extract())
            brand = if_available_strip(response.xpath('//*[@id="prodDetails"]//td[text()="Brand"]/following::td[@class="value"][1]/text()').extract())
            return_policy = if_available_strip(response.xpath('//div[@data-name="RETURNS_POLICY"]//span[@class="a-size-small"]/text()').extract())
            warranty = if_available_strip(response.xpath('//div[@data-name="WARRANTY"]//span[@class="a-size-small"]/text()').extract())
            pay_on_delivery = if_available_strip(response.xpath('//div[@data-name="PAY_ON_DELIVERY"]//span[@class="a-size-small"]/text()').extract())
            amazon_delivered = if_available_strip(response.xpath('//div[@data-name="AMAZON_DELIVERED"]//span[@class="a-size-small"]/text()').extract())
            cart_count = int(if_available_strip(response.xpath('//*[@id="nav-cart-count"]/text()').extract()))
            features = response.xpath('//*[@id="feature-bullets"]/ul/li//text()').extract()
            features = "||".join(feature.strip() for feature in features)
            num_offers = if_available_strip(response.xpath('//*[@id="olp-sl-new"]/span[@class="olp-padding-right"]/a/text()').extract())
            lowest_price = price_extract(response.xpath('//*[@id="olp-sl-new"]//span[@class="a-color-price"]/text()').extract())
            weight = if_available_strip(response.xpath('//*[@id="prodDetails"]//td[contains(translate(text(),"WEIGHT","weight"),"weight")]/following::td[@class="value"][1]/text()').extract())
            model = if_available_strip(response.xpath('//*[@id="prodDetails"]//td[contains(text(),"Model")]/following::td[@class="value"][1]/text()').extract())
